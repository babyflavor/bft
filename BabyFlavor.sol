...
contract bft is Context, IERC20, Ownable, Manageable {
    ...
    uint private startingSupply = 100_000_000; //100 Million, underscores aid readability
    ...
    string private _name = "BabyFlavor";
    ...
    uint256 public _marketingFee = 40; // 4% divisor 1000
    ...
    address payable private _marketingWallet = payable(0x165c4CdAca3034486265D18a9665C19035B33614);
    
    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled = false;
    
    // Max TX amount is 0.8% of the total supply.
    uint256 private maxTxPercent = 8; // Less fields to edit
    ...
    mapping (address => AdvancedFee) private _advancedFee;
    ...
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee <= 3); // Prevents owner from abusing fees.
        _liquidityFee = liquidityFee;
    }

    function setMarketingFeePercent(uint256 marketingFee) external onlyOwner() {
        require(marketingFee <= 40); // Prevents owner from abusing fees.
...
,        address to,
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        //check if there is any locked bnb left from rounding errors
    ...
    function checkDoubleTaxAmt(address sender, address recipient, uint256 amount) internal {
        if (recipient == uniswapV2Pair) {

            if(block.timestamp - _advancedFee[sender].lastSellTime > 300) {
                _advancedFee[sender].sellCounter=1;
            } else {
                _advancedFee[sender].sellCounter+=1;
            }
            
            _advancedFee[sender].lastSellTime=block.timestamp;
            
            if (amount > _doubleTaxAmt || _advancedFee[sender].sellCounter>2) {
                doubleTaxBool = true;
            ...
            if(_advancedFee[recipient].buyCounter>0) {
                
                _advancedFee[recipient].buyCounter=0;
                
                if(amount >= _advancedFee[recipient].lastBuySum) {
                    _advancedFee[recipient].lastBuySum=amount;
                    ...
                    _taxFee = _taxFee.div(2);
                    _liquidityFee = _liquidityFee.div(2);
                    _marketingFee = _marketingFee.div(2);
                ...
                _advancedFee[recipient].lastBuySum=amount;
                _advancedFee[recipient].buyCounter+=1;
            }
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(!takeFee)
            ...
            checkDoubleTaxAmt(sender, recipient, amount);
